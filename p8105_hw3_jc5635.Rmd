...
The `instacart` dataset contains `nrow(instacart)` rows and `r ncol(instacart)` columns, such as `order_id`, `product_id`, `user_id`, `order_number`, and `product_name`.
...
# note: total activity counts for Saturday in weeks 4 and 5 are 1440 each (i.e. number of minutes in a day; may merit further investigation) 
```
